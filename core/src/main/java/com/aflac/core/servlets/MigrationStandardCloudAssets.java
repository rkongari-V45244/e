...
@Component(service = { Servlet.class }, property = { SLING_SERVLET_METHODS + "=POST",
		SLING_SERVLET_EXTENSIONS + "=json", SLING_SERVLET_PATHS + "=/bin/StandardCloud" })

public class MigrationStandardCloudAssets extends SlingAllMethodsServlet {

	private static final long serialVersionUID = 1L;

	private static final String SERVICE_USER_ACCOUNT = "aflac-service-user";

	private transient Logger log = LoggerFactory.getLogger(MigrationStandardCloudAssets.class);
	...
	public void doPost(final SlingHttpServletRequest request, final SlingHttpServletResponse response)
		...
		try {
			...
			MigrationUtil.createStandardAsset(fileStream, dataJSONString, fileCreateName,resourceResolver);
